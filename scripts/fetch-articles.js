...
const REWRITE_SYSTEM_PROMPT =
  'Ты редактор медицинского портала о простатите, урологии и мужском здоровье. ' +
  `Напиши полноценную статью объёмом ${REWRITE_MIN_CHARS}–${REWRITE_MAX_CHARS} символов (4–6 абзацев). ` +
  'Сохрани все медицинские факты, цифры, названия процедур и рекомендации из оригинала. Не выдумывай диагнозы и методы лечения, которых нет в исходнике. ' +
  'Стиль: понятный, спокойный, информативный. Добавь дисклеймер, что материал не заменяет консультацию врача. Язык русский. Без markdown и заголовков.';
...
const parser = new Parser({
  timeout: 20000,
  headers: { 'User-Agent': 'UroHealthBot/1.0 (+https://github.com/prostate-news)' },
});

const TOPIC_KEYWORDS = [
  'простат', 'уролог', 'аденом', 'предстательн', 'мочеиспуск', 'мочевой', 'мочеполов',
  'эректил', 'потенц', 'андролог', 'импотенц', 'тестостерон', 'андропауз',
  'пса', 'биопси', 'уретр', 'цистит', 'недержан', 'простатит', 'мужск', 'полов',
  'сексуальн', 'либидо', 'сперм', 'эякуля', 'мочев', 'пузыр', 'почеч', 'камн',
];
...
const EXCLUDE_KEYWORDS = [
  ...
  'шоу-биз', 'развод', 'беремен', 'гинеколог', 'акушер', 'роды', 'маммолог', 'косметолог',
  'маникюр', 'пластическ', 'рецепт', 'блюд', 'готов', 'кухн', 'кулинар', 'десерт', 'выпеч',
  'детск', 'педиатр', 'новорожден', 'школьник', 'подростк', 'вакцин', 'грипп', 'ковид',
];

const CONTENT_ONLY_EXCLUDE = ['гинеколог', 'беремен', 'акушер', 'маммолог', 'педиатр'];

const RELAXED_TITLE_PATTERNS = [
  /простат/i, /уролог/i, /аденом/i, /предстательн/i, /мочеиспуск/i, /мочев/i,
  /эректил/i, /потенц/i, /андролог/i, /простатит/i, /мужск.{0,6}здоров/i,
  /тестостерон/i, /пса\b/i, /уретр/i, /цистит/i, /недержан/i,
];

const STRONG_TOPIC_KEYWORDS = [
  'простат', 'уролог', 'аденом', 'предстательн', 'простатит', 'мочеиспуск',
  'эректил', 'потенц', 'андролог', 'тестостерон', 'мочевой', 'уретр',
];
...
function titleMatchesHealthTopic(title) {
...
const CATEGORY_RULES = [
  [/простатит|простат|аденом|предстательн|пса\b|биопси/i, 'Простатит и аденома'],
  [/уролог|уретр|мочеиспуск|мочевой|цистит|недержан|камн/i, 'Урология'],
  [/эректил|потенц|импотенц|тестостерон|андролог|андропауз|либидо/i, 'Мужское здоровье'],
  [/лечен|профилакт|препарат|операци|терапи|диагност|обследован/i, 'Лечение и профилактика'],
  [/врач|совет|рекоменд|симптом|признак|осложнен/i, 'Советы врачей'],
];
...
function isTopicRelevant(title, text, source, { relaxed = false, broad = false } = {}) {
  ...
  if (broad) {
    return (
      titleMatchesHealthTopic(titleLower) &&
...
function detectCategory(title, text, defaultCategory) {
  ...
  return defaultCategory || 'Урология';
}
...
const CATEGORY_MAP = {
  'Простатит и аденома': 'Простатит и аденома',
  'Урология': 'Урология',
  'Мужское здоровье': 'Мужское здоровье',
  'Лечение и профилактика': 'Лечение и профилактика',
  'Советы врачей': 'Советы врачей',
};

const FALLBACK_TOPICS = [
  { title: 'Хронический простатит: основные симптомы и когда обращаться к урологу', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Аденома простаты: как отличить возрастные изменения от патологии', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Профилактика простатита: простые привычки для мужского здоровья', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Анализ ПСА: что означают цифры и когда нужна биопсия', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Застойный простатит: причины, симптомы и тактика лечения', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Частое мочеиспускание у мужчин: когда это признак проблем с простатой', category: 'Урология', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Тур простаты: показания, этапы восстановления и ограничения', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Эректильная дисфункция и простатит: есть ли связь', category: 'Мужское здоровье', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Массаж простаты: показания и противопоказания', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Хроническая тазовая боль у мужчин: как отличить от простатита', category: 'Урология', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Питание при простатите: что включить и что исключить из рациона', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Острый простатит: первые признаки и неотложная помощь', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Калькулезный простатит: что это и как лечат камни в простате', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Недержание мочи после операции на простате: что считается нормой', category: 'Урология', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Антибиотики при простатите: как правильно проходить курс лечения', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Физическая активность при простатите: что полезно, а что вредно', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Алкоголь и простатит: влияет ли на обострения', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Сексуальная активность при хроническом простатите: рекомендации урологов', category: 'Мужское здоровье', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'МРТ простаты: когда назначают и что показывает исследование', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Трансректальное УЗИ простаты: как подготовиться к процедуре', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Гормональные изменения у мужчин после 50 и здоровье простаты', category: 'Мужское здоровье', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Простатит у молодых мужчин: почему болеют в 25–35 лет', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Хронический простатит и бесплодие: есть ли связь', category: 'Мужское здоровье', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Народные средства от простатита: что говорит официальная медицина', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Когда аденому простаты лечат без операции', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Лазерная vaporization простаты: плюсы и минусы метода', category: 'Лечение и профилактика', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Цинк и селен для здоровья простаты: нужны ли добавки', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Как подготовиться к первому визиту к урологу', category: 'Советы врачей', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Обострение простатита зимой: почему случается и как снизить риск', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
  { title: 'Бактериальный и абактериальный простатит: в чём разница', category: 'Простатит и аденома', source_name: 'ПростаНьюс', source_url: 'https://prostanews.space/' },
];
...
function parseBrokenRssItems(xml) {
  const items = [];
  for (const block of xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)) {
    ...
    items.push({
      ...
      content: plain,
      contentSnippet: plain.slice(0, 500),
    });
  }
  return { items };
}

async function fetchFeedWithRetry(url, sourceName, encoding, brokenXml) {
  return withRetry(
    async () => {
      if (brokenXml || (encoding && encoding !== 'utf-8' && encoding !== 'utf8')) {
        const res = await axios.get(url, {
          responseType: brokenXml ? 'text' : 'arraybuffer',
          timeout: 20000,
          headers: { 'User-Agent': 'UroHealthBot/1.0 (+https://github.com/prostate-news)' },
          maxRedirects: 10,
        });
        let xml = brokenXml
          ? res.data
          : new TextDecoder(encoding).decode(res.data);
        if (brokenXml) {
          return parseBrokenRssItems(xml);
        }
  ...
  async function processFeedItems(relaxed = false, broad = false) {
    ...
    for (const source of sources) {
      ...
      try {
        ...
        const feed = await fetchFeedWithRetry(source.url, source.name, source.encoding, source.brokenXml);
        ...
        for (const item of items) {
          ...
          let itemDate = parseItemDate(item);
          if (!itemDate && source.allowNoDate) {
            itemDate = new Date();
          }
          if (!itemDate) {
